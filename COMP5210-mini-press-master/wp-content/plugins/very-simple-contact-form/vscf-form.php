...
function vscf_shortcode($vscf_atts) {
	...
	$email_form = '<form class="vscf" id="vscf" method="post" '.$action.'>
		...
		<div class="form-group">
			<label for="vscf_captcha">'.sprintf(esc_attr($captcha_label), $_SESSION['vscf-rand']).': <span class="'.(isset($error_class['form_captcha']) ? "error" : "hide").'" >'.esc_attr($error_captcha_label).'</span></label>
		...
		<div class="form-group">
			<button type="submit" name="vscf_send" id="vscf_send" class="btn btn-primary">'.esc_attr($submit_label).'</button>
		</div>
	</form>';
